...
func _on_HealthPack_body_enter( body ):
	...
	if (body.has_method("die_p1") and Game.health_p1 < Game.health_limit and Game.health_p1 > 0):
		Game.health_p1 += 50
		if (Game.health_p1 > Game.health_limit):
			Game.health_p1 = Game.health_limit
		queue_free()
	elif (body.has_method("die_p2") and Game.health_p2 < Game.health_limit and Game.health_p2 > 0):
		Game.health_p2 += 50
